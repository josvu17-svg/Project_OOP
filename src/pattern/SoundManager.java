 ...
 * Supports independent volume control for SFX and BGM.
 */
public class SoundManager {
    ...
    private static float sfxVolume = 0.8f;  // 0.0 - 1.0
    private static float bgmVolume = 0.6f;  // 0.0 - 1.0

    private static Thread bgmThread;
    private static volatile boolean bgmRunning = false;
    private static volatile SourceDataLine bgmLine = null;
    ...
    public static void setBgmVolume(float v) {
        ...
        // Apply to running line via FloatControl
        if (bgmLine != null && bgmLine.isOpen()) {
            try {
                FloatControl ctrl = (FloatControl) bgmLine.getControl(FloatControl.Type.MASTER_GAIN);
    ...
    public static void stopBgm() {
        bgmRunning = false;
        if (bgmLine != null) { bgmLine.stop(); bgmLine.flush(); }
        if (bgmThread != null) { bgmThread.interrupt(); bgmThread = null; }
    }
    ...
    private static void playBgmOnce() throws Exception {
        ...
        try (BufferedInputStream bis = new BufferedInputStream(is);
            ...
            bgmLine = (SourceDataLine) AudioSystem.getLine(info);
            bgmLine.open(target);
            ...
            int n;
            while (bgmRunning && bgmEnabled && (n = converted.read(buf)) != -1) {
                bgmLine.write(buf, 0, n);
            }
            bgmLine.drain();
            bgmLine.close();
            converted.close();
    ...
    // ── SFX ───────────────────────────────────────────────────
    public static void playLock()     { play(() -> tone(180, 60, 0.25f * sfxVolume, WaveType.SQUARE, true)); }
    public static void playMove()     { play(() -> tone(220, 25, 0.12f * sfxVolume, WaveType.SQUARE, true)); }
    public static void playRotate()   { play(() -> tone(300, 30, 0.15f * sfxVolume, WaveType.SQUARE, true)); }
    public static void playHardDrop() { play(() -> { sweep(800,200,60, 0.4f*sfxVolume); tone(150,80,0.3f*sfxVolume,WaveType.SQUARE,true); }); }

    public static void playSingle() {
        play(() -> { tone(400,60,0.35f*sfxVolume,WaveType.SINE,false); tone(600,80,0.30f*sfxVolume,WaveType.SINE,false); });
    }
    public static void playDouble() {
        play(() -> { tone(400,50,0.35f*sfxVolume,WaveType.SINE,false); tone(600,50,0.35f*sfxVolume,WaveType.SINE,false); tone(800,100,0.40f*sfxVolume,WaveType.SINE,false); });
    }
    public static void playTriple() {
        play(() -> { tone(400,40,0.35f*sfxVolume,WaveType.SINE,false); tone(600,40,0.35f*sfxVolume,WaveType.SINE,false); tone(800,40,0.40f*sfxVolume,WaveType.SINE,false); tone(1000,120,0.45f*sfxVolume,WaveType.SINE,false); });
    }
    public static void playTetris() {
        play(() -> { tone(523,80,0.5f*sfxVolume,WaveType.SINE,false); tone(659,80,0.5f*sfxVolume,WaveType.SINE,false); tone(784,80,0.5f*sfxVolume,WaveType.SINE,false); tone(1047,200,0.6f*sfxVolume,WaveType.SINE,false); noise(100,0.2f*sfxVolume); });
    }
    public static void playLevelUp() {
        play(() -> { tone(523,80,0.4f*sfxVolume,WaveType.SINE,false); tone(659,80,0.4f*sfxVolume,WaveType.SINE,false); tone(784,80,0.4f*sfxVolume,WaveType.SINE,false); tone(1047,150,0.5f*sfxVolume,WaveType.SINE,false); });
    }
    public static void playGameOver() {
        play(() -> { tone(440,150,0.4f*sfxVolume,WaveType.SINE,false); tone(349,150,0.4f*sfxVolume,WaveType.SINE,false); tone(294,150,0.4f*sfxVolume,WaveType.SINE,false); tone(220,400,0.5f*sfxVolume,WaveType.SINE,false); });
    }
