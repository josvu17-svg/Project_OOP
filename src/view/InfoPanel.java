...
public class InfoPanel extends JPanel {
    ...
    public InfoPanel(GameModel model) {
        ...
        addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                if (gearBtnRect.contains(e.getPoint()) && onSettings != null) onSettings.run();
            }
            @Override public void mouseExited(MouseEvent e) { gearHover=false; repaint(); }
        });
        addMouseMotionListener(new MouseAdapter() {
            @Override public void mouseMoved(MouseEvent e) {
                boolean was=gearHover; gearHover=gearBtnRect.contains(e.getPoint());
    ...
    public void setOnSettings(Runnable r)  { onSettings = r; }
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (String ctrl : controls) { g2.drawString(ctrl, 15, y); y += 17; }
    }
    ...
    private int drawNextPiece(Graphics2D g, int startY) {
        ...
        int minX=Integer.MAX_VALUE,maxX=Integer.MIN_VALUE,minY2=Integer.MAX_VALUE,maxY2=Integer.MIN_VALUE;
        for(int[] b:blocks){minX=Math.min(minX,b[0]);maxX=Math.max(maxX,b[0]);minY2=Math.min(minY2,b[1]);maxY2=Math.max(maxY2,b[1]);}
        int pw=(maxX-minX+1)*PREVIEW_CELL,ph=(maxY2-minY2+1)*PREVIEW_CELL;
        int ox=(getWidth()-pw)/2-minX*PREVIEW_CELL;
        g.setColor(new Color(45,45,45));
        g.fillRoundRect(10,startY-5,getWidth()-20,ph+15,8,8);
        for(int[] b:blocks){
            int px=ox+b[0]*PREVIEW_CELL,py=startY+(b[1]-minY2)*PREVIEW_CELL;
    ...
    public static int getCellSize() { return BoardPanel.getCellSize(); }
}
